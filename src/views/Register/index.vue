...
export default {
  components: { PicCode, VerifityCode, cityChoose },
  setup(props, context) {
    // 定义表单绑定的数据
    ...
    const regFormRules = reactive({
      ...
      surePwd: [
        { validator: val => val === regForm.pwd, message: "两次密码不一致,请确认后重新输入" }
      ]
    })
    // 接收短信验证码
    ...
    return { vcode, regForm, regFormRules, surePwd, changePicCode, changeCodeFn, onSubmit }
  }
}
</script>
