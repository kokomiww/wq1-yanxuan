  ...
  <div class="cart-wrapper">
    ...
    <div class="cart-container">
      <div class="cart-container-box">
        ...
        <ul
          class="cart-list"
          v-if="$store.getters.carts.length>0"
        >
          <van-swipe-cell
            v-for="item in $store.getters.carts"
            :key="item.id"
          >
            <li>
              <div class="cart-left">
    ...
    <div class="cart-footer">
      ...
      <div class="footer-right">
        <div class="total-price">
          ...
          <span class="totla-price-num">￥{{$store.getters['cart/selectedAmount'].toFixed(2)}}</span>
        </div>
        <div
          class="order-btn"
          v-if="isShow"
...
export default {
  ...
  setup() {
    ...
    const checkAll = computed({
      // 判断购物车中是否有所商品
      get() {
        if (store.getters.carts.length) {
          // 判断购物车中所有商品是否全部被选择，只要有一个不选中 全选按钮就不选中
          return store.getters.carts.every((item) => item.checked)
        } else {
          return false
        }
      },
    ...
    const order = () => {
      ...
      if (sessionStorage.getItem('token')) {
        // 从购物车列表中筛选出选中的商品
        const orderList = store.state.cart.cartList.filter(item => item.checked)
        if (orderList.length > 0) {
          // 添加到vuex中，跳转到订单列表
          store.commit('order/order', [...orderList])
          router.push('/orderlist')
        } else {
          Notify({ type: "warning", message: "你还没有选择任何商品" })
        }
      } else {
        Notify("你还没有登陆，请登录")
        router.push('/login')
