...
/*
接收一个参数 类型是object
该对象必须包含mobile key picCode
该组件对外提供一个change事件,对外传递短信验证码
该组件提供一个validateCode方法,用来校验短信验证码是否正确,返回值为true校验成功
*/
export default {
  props: {
    data: {
  ...
  setup(props, { emit }) {
    ...
    const getCode = async () => {
      // 获取短信验证码
      await getVerifityCode({ mobile: props.data.mobile, key: props.data.key, picCode: props.data.picCode })
      // 提示成功
